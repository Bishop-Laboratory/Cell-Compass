...
# Load TPMs processed from sequencing
proc_tpms <- read_csv("tpms.csv")
```
...
```{r}
select_cols <- colnames(proc_tpms)[colnames(proc_tpms) %in% metadata$run_id]

select_tpms <- proc_tpms %>%
  #filter(gene_id %in% supp_tpms$`Gene ID`) %>%
  select(all_of(select_cols))

# Log transform data for PCA
tpms_mat <- select_tpms %>% as.matrix() %>% t()
trans_mat <- log2(1 + (tpms_mat))
